...
export class NodeGraphPanel {
	...
	public static createOrShow(extensionUri: vscode.Uri) {
		...
		if (NodeGraphPanel.currentPanel) {
			NodeGraphPanel.currentPanel._panel.reveal(column);
			return;
		}

		// Otherwise, create a new panel.
		const panel = vscode.window.createWebviewPanel(
			NodeGraphPanel.viewType,
			'Node Graph',
			column || vscode.ViewColumn.One,
			getWebviewOptions(extensionUri),
		);

		NodeGraphPanel.currentPanel = new NodeGraphPanel(panel, extensionUri);
	}
	...
	private _update() {
		...
		this._panel.title = 'NodeGraph-TheONE';
